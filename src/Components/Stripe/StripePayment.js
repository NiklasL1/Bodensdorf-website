...
const StripePayment = ({handleClose}) => {
    ...
    const {startDate, endDate, arrayOfDates, prepaymentCost, restpaymentCost} = useContext(BookingLogicContext)
    ...
    const makePayment = (token) => {        
        ...
        return fetch(`https://bodensdorf-server.herokuapp.com/api/payment`, {
            method: "POST",
            headers: headers,
    ...
    return (
        <div>
            <StripeCheckout
                ...
                token={makePayment}
                name={`Anzahlung für ${arrayOfDates ? (arrayOfDates.length-1) : null} Nächte`}
                // panelLabel="Summe"
                description={`${Math.round(restpaymentCost)} € Restzahlung 1 Monat vor Anreise`}
                locale= "de"
                currency="EUR"
