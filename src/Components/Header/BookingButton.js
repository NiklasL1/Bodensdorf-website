...
const BookingButton = ({handleClose}) => {
    ...
    const { create, list } = useContext(BookingsContext)
    const {totalBookingCost, prepaymentCost, restpaymentCost, startDate, endDate, extraPerson, available} = useContext(BookingLogicContext)
    ...
    useEffect(() => {
        if(succeeded) {            
            ...
            setSucceeded(false)
            Swal.fire({
                title: "Buchung war erfolgreich. Vielen Dank!",
                icon: "success"
            })
            handleClose() 
        }                         
    ...
    }, [totalBookingCost, prepaymentCost, restpaymentCost, extraPerson, data._id])
