...
const BookingLogicContextProvider = ({ children }) => {
    ...
    const calculateDaysCost = () => {
        ...
        const offSeason = (date) => {
            ...
            if (startOff < endOff) {
                if (moment(date).format('MM') >= startOff && moment(date).format('MM') <= endOff) {
                    bookingCostArray.push(b.offSeasonPrice)
                    // console.log('offseasonPrice')                    
                }
            } else if (startOff > endOff) {
                if ((moment(date).format('MM') >= startOff && moment(date).format('MM') <= 12) || (moment(date).format('MM') >= 1 && moment(date).format('MM') <= endOff)) {
        ...
        const preSeason = (date) => {
            ...
            if (startPre < endPre) {
                if (moment(date).format('MM') >= startPre && moment(date).format('MM') <= endPre) {
                    // console.log('preseasonPrice')
                    bookingCostArray.push(b.preSeasonPrice)
                }
            } else if (startPre > endPre) {
                if ((moment(date).format('MM') >= startPre && moment(date).format('MM') <= 12) || (moment(date).format('MM') >= 1 && moment(date).format('MM') <= endPre)) {
        ...
        const mainSeason = (date) => {
            ...
            if (startMain < endMain) {
                if (moment(date).format('MM') >= startMain && moment(date).format('MM') <= endMain) {
                    // console.log('mainseasonPrice')
                    bookingCostArray.push(b.mainSeasonPrice)
                }
            } else if (startMain > endMain) {
                if ((moment(date).format('MM') >= startMain && moment(date).format('MM') <= 12) || (moment(date).format('MM') >= 1 && moment(date).format('MM') <= endMain)) {
        ...
        const aftSeason = (date) => {
            ...
            if (startAft < endAft) {
                if (moment(date).format('MM') >= startAft && moment(date).format('MM') <= endAft) {
                    // console.log('aftseasonPrice')
                    bookingCostArray.push(b.aftSeasonPrice)
                }
            } else if (startAft > endAft) {
                if ((moment(date).format('MM') >= startAft && moment(date).format('MM') <= 12) || (moment(date).format('MM') >= 1 && moment(date).format('MM') <= endAft)) {
        ...
        if (extraPerson === true) {
            ...
            let fullCost = Math.round((cost + b.cleaningFee) * 1.25)
            // console.log(fullCost)
            setTotalBookingCost(fullCost)
            setPrepaymentCost(fullCost*0.15)
            setRestpaymentCost(fullCost*0.8499)
    ...
    const showDates = (lang) => {
        ...
        if (lang === "DE")
        return (
            `${moment(arrivalDate).format('DD/MM/YYYY')} - ${moment(departureDate).format('DD/MM/YYYY')}`
        )
        if (lang === "EN")
        return (
            `${moment(arrivalDate).format('MM/DD/YYYY')} - ${moment(departureDate).format('MM/DD/YYYY')}`
        )
    }

    return (
        <BookingLogicContext.Provider value={{ 
            totalBookingCost,
            setTotalBookingCost,
            available,
            setAvailable,
